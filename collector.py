# -*- coding: utf-8 -*-
"""
Czech Media Tracker — Collector + Translator + Topic Splitter
...
SOURCES: Dict[str, Dict] = {
    ...
    "České noviny / ČTK - Economy": {
        "feeds": [
            "https://www.ceskenoviny.cz/sluzby/rss/ekonomika.php",
        ],
    },
    ...
    "Radio Prague International (EN)": {
        "feeds": [
            "https://english.radio.cz/rcz-rss/en",
        ],
    },
}
...
def canonical_text_for_similarity(
    article: dict
) -> str:

    return normalize_space(
        f"{article.get('title_raw', '')} "
        f"{article.get('lead_raw', '')}".lower()
    )
...
def classify_topics(
    ...
    # Search both original Czech text
    # and translated English text
    txt = (
        f"{article.get('title_raw', '')} "
        f"{article.get('lead_raw', '')} "
        f"{article.get('title_en', '')} "
        f"{article.get('lead_en', '')}"
    ).lower()
...
def choose_better_article(
    ...
    # Slight preference for newer copy
...
def main() -> int:
    ...
    print(
        ...
        f"| keep_hours={KEEP_HOURS}"
    )

    print(
        f"[INFO] Translator available: "
        f"{HAS_TRANSLATOR}"
    )
    ...
    merged_raw_for_latest = (
        existing_latest
        + collected
    )

    merged_raw_for_archive = (
        existing_archive
        + collected
    )
    ...
    # No Russia-only filter.
    # All selected Czech news feeds
    # are retained.
    ...
    print(
        f"[INFO] Latest articles: "
        f"{len(latest_filtered)}"
    )

    print(
        f"[INFO] Archive articles: "
        f"{len(archive_filtered)}"
    )
    ...
    latest_classified = [
        classify_topics(a)
        for a in latest_filtered
    ]

    archive_classified = [
        classify_topics(a)
        for a in archive_filtered
    ]
